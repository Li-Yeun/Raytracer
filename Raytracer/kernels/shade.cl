...
__global float4* albedos, __global float4* primNorms, __global float* sphereInvrs, int sphereStartIdx, int sphereCount,                                                          // Primitives
...
{   
    ...
    float4 BRDF = albedos[primIdxs[threadId]] / M_PI_F;
    ...
    float p = clamp(max(albedos[primIdxs[threadId]].z, max(albedos[primIdxs[threadId]].x, albedos[primIdxs[threadId]].y)), 0.0f, 0.9f);
    //printf("float: %f", p);
    if (p >= RandomFloat()) {
